...
import testutils._
import module.bpu.ras.RGmuRAS
import utils.{LogUtil, LogLevel}

class RGmuRASSpec extends AnyFreeSpec with Matchers with HasDebugPrint {
  "RGmuRAS should handle push, pop, commit and rollback correctly" in {
    LogUtil.setLogLevel(LogLevel.OFF)
    debugPrint = false
    simulate(new RGmuRAS(4)) { dut =>
